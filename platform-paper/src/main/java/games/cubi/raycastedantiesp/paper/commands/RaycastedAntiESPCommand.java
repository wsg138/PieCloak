...
public class RaycastedAntiESPCommand {
    ...
    @Executes("benchmark")
    void benchmarkCommand(int radius, int samples, CommandSender sender) throws CommandSyntaxException {
        Player player = senderPlayer(sender);
        if (player == null) {
            return;
        }
        benchmarkCommand(radius, samples, player);
    }
    ...
    void benchmarkCommand(int radius, int samples, Player player) throws CommandSyntaxException {
        ...
        player.sendRichMessage("<gray>Running raycast benchmark: <white>" + clampedSamples
                + " <gray>samples within <white>" + clampedRadius + "<gray> blocks...");
        Bukkit.getAsyncScheduler().runNow(RaycastedAntiESP.get(), ignored -> {
            ...
            String result = "<white>Raycast benchmark complete: <green>" + clampedSamples
            ...
            Bukkit.getScheduler().runTask(RaycastedAntiESP.get(), () -> {
                if (player.isOnline()) {
                    player.sendRichMessage(result);
                }
            });
        });
    }
    ...
    static class TraceCommands {
        ...
        void traceLookedAtEntity(CommandSender sender) {
            ...
            traceLookedAtEntity(player);
        }

        @Executes("entity")
        void traceLookedAtEntity(Player player) {
            ...
            if (hit == null) {
                Entity closestAny = closestLookedAtEntity(player);
                if (closestAny == null) {
                    ...
                    player.sendRichMessage("<gray>Closest hit was <white>" + closestAny.getType().getKey()
                            + " <gray>entityID=<white>" + closestAny.getEntityId()
                            + " <gray>managed=<white>false");
                }
                return;
            }
            startEntityTrace(player, playerData, hit.getEntityId(), hit.getType().name());
        }
        ...
        void traceEntityById(int entityID, CommandSender sender) {
            ...
            traceEntityById(entityID, player);
        }

        @Executes("entity-id")
        void traceEntityById(int entityID, Player player) {
            PlayerData playerData = playerData(player);
            if (playerData == null) {
                return;
            }
            Entity bukkitEntity = SpigotConversionUtil.getEntityById(player.getWorld(), entityID);
            String label = bukkitEntity == null ? "entity-id-" + entityID : bukkitEntity.getType().name();
        ...
        void traceLookedAtBlockEntity(CommandSender sender) {
            ...
            traceLookedAtBlockEntity(player);
        }

        @Executes("block")
        void traceLookedAtBlockEntity(Player player) {
            PlayerData playerData = playerData(player);
            if (playerData == null) {
                return;
            }
            ...
            if (!(block.getState() instanceof TileState)) {
                player.sendRichMessage("<yellow>That block is not a Bukkit block entity: <white>" + block.getType().getKey());
                player.sendRichMessage("<gray>Trace not started. Look at a shulker, sign, bell, beacon, campfire, etc.");
                return;
            }
            ...
            if (tracked == null) {
                player.sendRichMessage("<yellow>This block entity is not currently managed for you. Logs will start if it becomes a managed target.");
            } else {
                player.sendRichMessage("<gray>Current tracked state: visible=<white>" + tracked.visible() + "<gray>, blockID=<white>" + tracked.blockID());
            }
        ...
        void traceDump(CommandSender sender) {
            ...
            if (lines.isEmpty()) {
                sender.sendMessage("No visibility trace lines have been recorded yet.");
                return;
            }
        ...
        void traceClear(CommandSender sender) {
            ...
            sender.sendMessage("Visibility trace file and recent in-memory trace lines cleared.");
        }
        ...
        void traceHelp(CommandSender sender) {
            ...
            sender.sendRichMessage("<green>/raesp trace file <gray>- Show the full trace file path");
            sender.sendRichMessage("<green>/raesp trace stop <gray>- Stop your active trace");
            sender.sendRichMessage("<green>/raesp trace clear <gray>- Clear the trace file");
        }
        ...
        private static void startEntityTrace(Player player, PlayerData playerData, int entityID, String label) {
            ...
            if (tracked == null) {
                ...
                player.sendRichMessage("<gray>If it should be hidden, make sure it is allowlisted and start the trace while close enough to see it.");
                return;
            }
        ...
        private static PlayerData playerData(Player player) {
            PlayerData playerData = PlayerRegistry.getInstance().getPlayerData(player.getUniqueId());
            if (playerData == null) {
                player.sendRichMessage("<red>No RaycastedAntiESP player data is available yet. Relog or wait a moment and try again.");
                return null;
            }
            return playerData;
        }
        ...
        private static Player senderPlayer(CommandSender sender) {
            ...
            sender.sendMessage("This trace command must be run by a player.");
            return null;
        }

        private static void sendTraceFile(CommandSender sender) {
            if (VisibilityTraceService.get().outputPath() == null) {
                sender.sendMessage("Trace file path is not initialised yet.");
                return;
            }
