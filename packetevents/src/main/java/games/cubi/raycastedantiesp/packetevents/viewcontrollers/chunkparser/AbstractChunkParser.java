...
abstract class AbstractChunkParser<D> implements ChunkParser {
    ...
    protected abstract void storeSection(BlockView blockView, UUID world, int chunkX, int sectionY,
            int chunkZ, D data);
    ...
    public final @Nullable Column parse(BlockView blockView, UUID world, Column column, int minimumSectionY) {
        ...
        for (int sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
            ...
            blockView.replaceChunkSectionBlockEntities(
                    world, chunkX, sectionY, chunkZ, toBitset(managed));
        }

        blockView.pruneTileEntitiesAbsentFromChunkSections(
                world, chunkX, chunkZ, minimumSectionY, sections.length, managedBySection);
        ...
        for (int index = 0; index < sourceTileEntities.length; index++) {
            ...
            boolean strip = processTileEntity(blockView, world, blockOriginX, blockOriginZ, sections,
                    minimumSectionY, managedBySection, tileEntity, key);
    ...
    private static @Nullable OccludingChunkData toBitset(@Nullable long[] words) {
    ...
    private boolean processTileEntity(BlockView blockView, UUID world, int blockOriginX, int blockOriginZ,
        ...
        if (sectionIndex >= 0 && sectionIndex < sections.length) {
            ...
            if (managed != null && (managed[packed >>> 6] & 1L << packed) != 0L) {
                ...
                if (state != null) {
                    replayData(state).setBlockEntityData(
                            BlockEntityTypes.getById(clientVersion, tileEntity.getType()), tileEntity.getNBT());
                    return !state.visible();
                }
                Logger.warning("Managed chunk block entity was missing from tracked state at "
        ...
        if (sectionIndex < 0 || sectionIndex >= sections.length) {
            Logger.warning("Received chunk block entity outside authoritative section data at "
                    + blockX + "," + blockY + "," + blockZ, 3, AbstractChunkParser.class);
            return true;
        }
    ...
    private static TrackedTileEntity<PacketEventsTileEntityReplayData> tracked(
            BlockView blockView, UUID world, MutableBlockSpatialImpl position) {
        return (TrackedTileEntity<PacketEventsTileEntityReplayData>)
                blockView.getTrackedTileEntity(world, position);
    }

    private static PacketEventsTileEntityReplayData replayData(
    ...
    private Column copyColumn(Column column, TileEntity[] tileEntities) {
        if (modernHeightmaps) {
            return new Column(column.getX(), column.getZ(), column.isFullChunk(),
                    column.getChunks(), tileEntities, column.getHeightmaps());
        }
        return new Column(column.getX(), column.getZ(), column.isFullChunk(),
                column.getChunks(), tileEntities, column.getHeightMaps());
    }
